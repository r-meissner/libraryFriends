...
const OutgoingFriendRequestPage = () => {
  ...
  useEffect(() => {
    const fetchOutgoingFriendRequests = async (activeUserId) => {
      try {
        setLoading(true);
        const outgoingFriendRequests = await getOutgoingFriendRequestsOfUser(
          activeUserId
        );
                    ...
                    outgoingFriendRequests.map((outgoingFriendRequest) => (
                      ...
                      >
                        ...
                        <div className="col-span-4 flex justify-center items-center">
                          <Link
                            to={`/profile/${outgoingFriendRequest.targetUser._id}`}
                          >
                            <h2>{outgoingFriendRequest.targetUser.userName}</h2>
                          </Link>
                        </div>
                        ...
                        <div className="col-span-2 flex justify-center items-center">
                          {outgoingFriendRequest.status === "pending" ? (
                            <div className="badge badge-primary badge-lg">
                              waiting for
                              {outgoingFriendRequest.targetUser.userName}'s
                              answer
                            </div>
                          ) : (
                            <div className="badge badge-primary badge-lg">
